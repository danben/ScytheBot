...
class Simulator:
    ...
    @classmethod
    def init(cls, num_workers, worker_id, env, num_players, c, simulations_per_choice, evaluator_conn, learner_queue):
...
def worker(wid, num_workers, evaluator_conns, learner_queue, num_players, simulations_per_choice):
    ...
    # of evaluator connections. We need to get all of the shared memory blocks and then give each
    # simulator only the slices that it needs to do its job.
    sims = [Simulator.init(num_workers, wid, shm_envs[i], num_players, c=0.8,
                           simulations_per_choice=simulations_per_choice, evaluator_conn=evaluator_conns[i],
